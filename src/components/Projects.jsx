      ...
      <div className="relative max-w-7xl mx-auto px-6" ref={ref}>
        ...
        >
          <div className="flex items-center gap-4 mb-4">
            ...
            <span className="font-mono text-slate-600 text-xs tracking-widest uppercase">What I've Built</span>
          </div>
          ...
          {projects.map((p, i) => {
            ...
            return (
              <motion.button
                ...
                whileHover={{ y: -2 }}
                className={`flex-shrink-0 flex items-center gap-3 px-6 py-3 rounded-lg border font-mono text-sm transition-all duration-300 ${
                  activeProject === i
              ...
              >
                <span>{p.emoji}</span>
                <span className="hidden md:inline">{p.type}</span>
                <span>{p.number}</span>
              </motion.button>
            );
          })}
              ...
              <div className="grid md:grid-cols-2 gap-12">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-8">
                    {current.tech.map((t) => (
                      <span key={t} className={`px-3 py-1.5 border rounded-sm font-mono text-xs ${c.tag}`}>
                        {t}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex gap-4">
                    <motion.button
                      ...
                      whileTap={{ scale: 0.97 }}
                      className={`px-6 py-3 ${c.accent} border ${c.border} font-mono text-xs tracking-widest uppercase hover:${c.bg} transition-all duration-300 rounded-sm`}
                    >
                      View Details →
                    </motion.button>
                  </div>
                ...
                <div>
                  ...
                  <div className="mt-8 flex items-end justify-end">
                    <span
                      ...
                      style={{ WebkitTextStroke: `1px rgba(${current.color === "cyan" ? "0,245,255" : current.color === "pink" ? "255,0,110" : "57,255,20"},0.08)` }}
                    >
                      {current.number}
                    </span>
                  </div>
